...
export default class List extends cc.Component {
    ...
    set selectedId(val: number) {
        ...
        switch (t.selectedMode) {
            case SelectedType.SINGLE: {
                ...
                t._selectedId = val;
                if (item) {
                    listItem = item.getComponent(this._component);
                    listItem.selected = true;
                }
                if (t._lastSelectedId >= 0 && t._lastSelectedId != t._selectedId) {
                    let lastItem: any = t.getItemByListId(t._lastSelectedId);
                    if (lastItem) {
                        lastItem.getComponent(this._component).selected = false;
                    }
                }
                if (t.selectedEvent) {
            ...
            case SelectedType.MULT: {
                ...
                if (!item)
                    return;
                let listItem = item.getComponent(this._component);
                if (t._selectedId >= 0)
                    t._lastSelectedId = t._selectedId;
                t._selectedId = val;
    ...
    private _ItemUpdateFunc:(item:cc.Node, list_id:number)=> void;
    ...
    private _renderCallBack: (item: cc.Node) => void = null;
    set renderCallBack(callBack: (item: cc.Node) => {}) {
        this._renderCallBack = callBack;
    }
    private _component:{ new(): ListItem } = ListItem;
    set itemComponent(compent:{ new(): ListItem }){
        this._component = compent;
    }
    onScrollEnd:()=>void = null;
    //初始化
    Init(compent:{ new(): ListItem },callBack: (item: cc.Node) => void,scrollEnd?: ()=> void,scaleX = 1,scaleY = 1){
        this._renderCallBack = callBack;
        this._component = compent;
    ...
    setTemplateItem(item: any) {
        ...
        if (t._resizeMode == cc.Layout.ResizeMode.CHILDREN)
            ...
            t._itemSize = cc.size(item.width* this.scaleX, item.height * this.scaleY);

        //获取ListItem，如果没有就取消选择模式
        let com = item.getComponent(this._component);
        let remove = false;
        if (!com)
            remove = true;
        // if (com) {
    ...
    _calcViewPos() {
        ...
        switch (this._alignCalcType) {
            ...
            case 3://单列VERTICAL（TOP_TO_BOTTOM）、网格HORIZONTAL（TOP_TO_BOTTOM）
                ...
                // this.elasticTop = this.node.height / 2;
    ...
    _calcItemPos(id: number) {
        let width: number, height: number, top: number, bottom: number, left: number, right: number, itemX: number,
            itemY: number;
        switch (this._align) {
            case cc.Layout.Type.HORIZONTAL:
    ...
    _onScrollEnded() {
        ...
        if(this.onScrollEnd)this.onScrollEnd.call(this);
    }
    ...
    _createOrUpdateItem(data: any) {
        ...
        if (!item) { //如果不存在
            ...
            let listItem: any = item.getComponent(this._component);
            ...
            if (this._renderCallBack) {
                this._renderCallBack.call(this,item,data.id % this._actualNumItems);
            } else if (this.renderEvent.target) {
                cc.Component.EventHandler.emitEvents([this.renderEvent], item, data.id % this._actualNumItems);
            }
            else if (this._ItemUpdateFunc) {
                this._ItemUpdateFunc(item, data.id % this._actualNumItems);
            }
        } else if (this._forceUpdate && (this.renderEvent || this._ItemUpdateFunc)) { //强制更新
            item.setPosition(cc.v2(data.x, data.y));
            this._resetItemSize(item);
    ...
    _createOrUpdateItem2(listId: number) {
        ...
        if (!item) { //如果不存在
            ...
            if (this._renderCallBack) {
                this._renderCallBack.call(this,item,listId);
            } else if (this.renderEvent.target) {
                cc.Component.EventHandler.emitEvents([this.renderEvent], item, listId);
            }
            else if (this._ItemUpdateFunc) {
                this._ItemUpdateFunc(item, listId);
            }
        } else if (this._forceUpdate && (this.renderEvent || this._ItemUpdateFunc)) { //强制更新
            item._listId = listId;
            if (listItem)
    ...
    updateItem(args: any) {
        ...
        for (let n: number = 0, len: number = args.length; n < len; n++) {
            ...
            if (item){
                ...
                if (this.renderEvent.target) {
                    cc.Component.EventHandler.emitEvents([this.renderEvent], item, listId % this._actualNumItems);
                }
                else if (this._ItemUpdateFunc) {
                    this._ItemUpdateFunc(item, listId % this._actualNumItems);
                }else if(this._renderCallBack){
    ...
    SetItemUpdateFunc(func:(item:cc.Node, list_id:number)=> void){
        this._ItemUpdateFunc = func;
    }
}